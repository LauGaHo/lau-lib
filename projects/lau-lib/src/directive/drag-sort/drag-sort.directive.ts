...
export class DragSortDirective implements AfterViewInit {
  ...
  public getRect() {
    ...
    for (let i = 0; i < this.parent.nativeElement.children.length; i++) {
      this.rects.push(this.parent.nativeElement.children[i].getBoundingClientRect());
    }
  }
  ...
  public handlePointerDown = (event: PointerEvent) => {
    ...
    // 更新 clone 的位置信息
    this.clone = {...this.clone, x: this.rects[i].left, y: this.rects[i].top};
  ...
  public handlePointerMove = (event: PointerEvent) => {
    if (this.isPointerDown) {
      ...
      for (let i = 0; i < this.rects.length; i++) {
        // 碰撞检测
        if (event.clientX > this.rects[i].left && event.clientX < this.rects[i].right &&
          event.clientY > this.rects[i].top && event.clientY < this.rects[i].bottom) {
          // 将当前元素记录为释放元素
          ...
          if (this.drag.ele !== this.drop.ele) {
            ...
            const dragRect = this.rects[this.drag.index];
            const lastDragRect = this.rects[this.drag.lastIndex];
            const dropRect = this.rects[this.drop.index];
            const lastDropRect = this.rects[this.drop.lastIndex];
